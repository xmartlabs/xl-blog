...
const MobileNavMenu = () => {
  ...
  return (
    <>
      <div className={classnames("w-[100%] absolute", { "h-[60rem] bg-neutral-100" : showMenu })}>
        {showTypeForm && <TypeForm onClick={() => setShowTypeForm(false)}/>}
        <div className={classnames("flex flex-row justify-end", { "hidden": showMenu })}>
          <button onClick={() => setShowMenu(!showMenu)} className="bg-transparent border-transparent mt-10 m-[2.5rem_5%_0_0]">
            <MenuMobileIcon />
          </button>
        </div>
        {showMenu && 
          <div>
            <div className="flex flex-row justify-end">
              <button
                ...
                className={classnames("flex flex-row justify-end", "bg-transparent border-transparent mt-10 m-[2.5rem_5%_0_0]")}
              >
                <CloseIcon className="fill-blue-one w-6" />
              </button>
            </div>
            <div className="m-[10%_0_0_5%]">
              <NavMenuElements className="flex flex-col ml-0 mb-6" openMenu={showMenu}/>
              <div className="mt-[3rem] font-primary font-bold">
                <h4 className="text-label-bold text-blue-one">Ready to partner?</h4>
                <button 
                  onClick={() => setShowTypeForm(true)} target="_blank" rel="noopener noreferrer"
                  className="
